...
describe 'RightScale::RightPopen' do
  ...
  [:sync, :async].each do |synchronicity|
    it "should redirect output #{synchronicity}" do
      ...
      runner = ::RightScale::RightPopen::Runner.new
      status = runner.method("run_right_popen3_#{synchronicity}".to_sym).call(command)
  ...
  it 'should return the right status' do
    ...
    runner = ::RightScale::RightPopen::Runner.new
    status = runner.run_right_popen3_async(command)
  ...
  it 'should correctly handle many small processes' do
    ...
    @started = 0
    run_cmd = Proc.new do
      runner.do_right_popen3_async(command) do |status|
        @completed += 1
        status.status.exitstatus.should == 0
        status.output_text.should == ''
        status.error_text.should == ''
  ...
  it 'should preserve interleaved output when yielding CPU on consumer thread' do
    ...
    puts
    stats = runner.run_right_popen3_async(command, :repeats=>repeats, :force_yield=>force_yield) do |status|
  ...
  it 'should setup environment variables' do
    ...
    status = runner.run_right_popen3_async(command, :env=>{ :__test__ => '42' })
    status.status.exitstatus.should == 0
  ...
  if is_windows?
    ...
    it 'should merge the PATH variable instead of overriding it' do
      command = "\"#{RUBY_CMD}\" \"#{File.expand_path(File.join(File.dirname(__FILE__), 'print_env.rb'))}\""
      runner = ::RightScale::RightPopen::Runner.new
      status = runner.run_right_popen3_async(command, :env=>{ 'PATH' => "c:/bogus\\bin" })
      status.status.exitstatus.should == 0
  ...
  it 'should run repeatedly without leaking resources' do
    ...
    command = "\"#{RUBY_CMD}\" \"#{File.expand_path(File.join(File.dirname(__FILE__), 'produce_output.rb'))}\" \"#{STANDARD_MESSAGE}\" \"#{ERROR_MESSAGE}\""
    runner = ::RightScale::RightPopen::Runner.new
    stats = runner.run_right_popen3_async(command, :repeats=>REPEAT_TEST_COUNTER)
  ...
  it 'should pass input to child process' do
    ...
    runner = ::RightScale::RightPopen::Runner.new
    status = runner.run_right_popen3_async(command, :input=>"42\n")
    status.status.exitstatus.should == 0
  ...
  pending 'not implemented for windows' if is_windows?
    ...
    runner = ::RightScale::RightPopen::Runner.new
    status = runner.run_right_popen3_async(command, :expect_timeout=>true, :timeout=>2)
