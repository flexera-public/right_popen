...
describe 'RightScale::RightPopen' do
  ...
  [:sync, :async].each do |synchronicity|
    it "should redirect output [#{synchronicity}]" do
    ...
    it "should return the right status [#{synchronicity}]" do
    ...
    it "should close all IO handlers, except STDIN, STDOUT and STDERR [#{synchronicity}]" do
      GC.start
      command = "\"#{RUBY_CMD}\" \"#{File.expand_path(File.join(File.dirname(__FILE__), 'produce_status.rb'))}\" #{EXIT_STATUS}"
      status = runner.run_right_popen3(synchronicity, command)
      status.status.exitstatus.should == EXIT_STATUS
    ...
    it "should preserve the integrity of stdout when stderr is unavailable [#{synchronicity}]" do
    ...
    it "should preserve the integrity of stderr when stdout is unavailable [#{synchronicity}]" do
      count = LARGE_OUTPUT_COUNTER
    ...
    it "should preserve interleaved output when yielding CPU on consumer thread [#{synchronicity}]" do
      lines = 11
      exit_code = 42
      repeats = 5
      force_yield = 0.1
    ...
    it "should preserve interleaved output when process is spewing rapidly [#{synchronicity}]" do
    ...
    it "should setup environment variables [#{synchronicity}]" do
    ...
    it "should restore environment variables [#{synchronicity}]" do
      begin
        ENV['__test__'] = '41'
        old_envs = {}
    ...
    if is_windows?
      ...
      it "should merge the PATH variable instead of overriding it [#{synchronicity}]" do
        command = "\"#{RUBY_CMD}\" \"#{File.expand_path(File.join(File.dirname(__FILE__), 'print_env.rb'))}\""
      ...
      it "should allow running bash command lines starting with a built-in command [#{synchronicity}]" do
      ...
      it "should support running background processes [#{synchronicity}]" do
        command = "(sleep 20)&"
        now = Time.now
        status = runner.run_right_popen3(synchronicity, command)
        finished = Time.now
    ...
    it "should support raw command arguments [#{synchronicity}]" do
    ...
    it "should run repeatedly without leaking resources [#{synchronicity}]" do
    ...
    it "should pass input to child process [#{synchronicity}]" do
    ...
    it "should run long child process without any watches by default [#{synchronicity}]" do
    ...
    it "should interrupt watched child process when timeout expires [#{synchronicity}]" do
      command = "\"#{RUBY_CMD}\" \"#{File.expand_path(File.join(File.dirname(__FILE__), 'sleeper.rb'))}\""
    ...
    it "should allow watched child to write files up to size limit [#{synchronicity}]" do
    ...
    it "should interrupt watched child at size limit [#{synchronicity}]" do
      ::Dir.mktmpdir do |watched_dir|
        command = "\"#{RUBY_CMD}\" \"#{File.expand_path(File.join(File.dirname(__FILE__), 'writer.rb'))}\" \"#{watched_dir}\""
    ...
    it "should handle child processes that close stdout but keep running [#{synchronicity}]" do
    ...
    it "should handle child processes that spawn long running background processes [#{synchronicity}]" do
